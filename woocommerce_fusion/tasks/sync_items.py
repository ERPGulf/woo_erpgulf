	...
	def push_wc_product(self, product_id: int, meta=None, **fields) -> dict:
		...
		try:
			...
			if resp.status_code not in (200, 201):
				frappe.log_error(f"Woo API error {resp.status_code}: {resp.text}")
				return {}
			return resp.json()
	...
	def update_woocommerce_product(
		...
		self.push_wc_product(
			...
			meta={
				...
				"branch_stock_1_branch": "qassim-branch",
				"branch_stock_1_stock_qty": 43.33,
				"branch_stock_2_branch": "riyadh-branch",
				"branch_stock_2_stock_qty": 53.33,
				"branch_stock": 3,
			}
		)
		self.woocommerce_product = wc_product
		# frappe.log_error(item.item.item_name)
		self.set_sync_hash()
